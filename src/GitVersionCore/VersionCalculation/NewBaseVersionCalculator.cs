...
namespace GitVersion.VersionCalculation
{
    public class BaseVersionCalculator : IBaseVersionCalculator
    {
        ...
        {
            //var fast = GetCalculatedBaseFast(context);
            var slow = GetCalculatedBaseSlow(context);
...
//            }
        ...
        private BaseVersion GetCalculatedBaseFast(GitVersionContext context)
        {
            ...
            {
                var versions = strategies
                    .Select(s => s.GetMaxVersion(context)).Where(v => v != null).ToList();
                return GetBaseVersion(context, versions);
            }
        }

        private BaseVersion GetCalculatedBaseSlow(GitVersionContext context)
        {
            using (Logger.IndentLog("Calculating base versions"))
            {
                var versions = strategies
                    .SelectMany(s => s.GetVersions(context)).ToList();

                return GetBaseVersion(context, versions);
            }
        }

        private BaseVersion GetBaseVersion(GitVersionContext context, List<BaseVersion> versions)
        {
            var baseVersions = versions
                .Where(v =>
                {
